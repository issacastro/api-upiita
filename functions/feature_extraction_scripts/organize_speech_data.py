#debería ayudar a los sistemas operativos Windows, Macs, Linux#
from pathlib import Path, PurePath
#saving labels
import csv
import random
from functions.feature_extraction_scripts.errors import DatasetMixing
...
def collect_audio_and_labels(data_path):
    '''
    espera que los archivos de onda estén en el subdirectorio: "datos
    se espera que las etiquetas sean los nombres de cada subdirectorio en "datos
    se espera que las identificaciones de los altavoces sean la primera sección de cada archivo WAV
    '''
    p = Path(data_path)
    waves = list(p.glob('**/*.wav'))
    # Remover los directorios con "_" al principio
...
def log_class4balance(max_num_per_class,class_max_samps,head_folder):
    #No necesito un nombre único: escribir en un archivo existente
    filename = '{}/features_log.csv'.format(head_folder)
    with open(filename, 'a', newline='') as f:
        w = csv.writer(f)
        info = {"Class with fewer samples":class_max_samps,"max number of samples per class":max_num_per_class}
    ...
    try:
        for train_index in train_indices[0]:
            if train_index in test_indices or train_index in val_indices:
                raise DatasetMixing("El índice {} de la clase {} está en múltiples datasets".format(val_index, label))
