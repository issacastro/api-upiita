'''
Estas son una serie de funciones que hice mientras trabajaba en un proyecto anterior (Mimic-Master).
Requería la cancelación del ruido y la detección del habla. Sucede que estas 
también ayudan a preparar el discurso para el aprendizaje profundo! 

Trabajaré en añadir explicaciones a cada una de las funciones; debería haberlo hecho cuando las hice... la retrospectiva es siempre 20-20.
...
from functions.feature_extraction_scripts.errors import NoSpeechDetected
...
def scale_noise(np_array,factor):
    '''
    Si se quiere reducir la amplitud a la mitad, el factor debe ser igual a 0,5
    '''
...
def get_speech_samples(samples, sr):
    try:
        ...
        if beg[1] == False or end[1] == False:
            raise NoSpeechDetected("No se detecta el habla")
...
######
def get_noise_samples(samples, sr):
    try:
        signal_length = len(samples)
        stft = wave2stft(samples,sr)
        energy = get_energy(stft)
        ...
        sample_end = int(perc_end*signal_length)
        samples_speech = samples[sample_start:sample_end]
        signal1= samples[0:sample_start]
        signal2= samples[sample_end:signal_length]
        silence = np.append(signal1,signal2)
...
def samps2stft(y, sr):
    if len(y)%2 != 0:
        y = y[:-1]
    #print("forma de las muestras: {}".format(y.shape))
    stft = librosa.stft(y)
    #print("forma de stft: {}".format(stft.shape))
    stft = np.transpose(stft)
    #print("forma transpuesta: {}".format(stft.shape))
    return stft
...
def stft2samps(stft,len_origsamp):
    ...
    ##print("forma transpuesta: {}".format(istft.shape))
...
def stft2power(stft_matrix):
    if stft_matrix is not None:
        if len(stft_matrix) > 0:
            ...
            raise TypeError("La matriz STFT está vacía. La función 'stft2power' necesita una matriz no vacía.")
    else:
        raise TypeError("La matriz STFT no existe. La función 'stft2power' necesita una matriz existente.")
    return None
...
def voice_activity_detection(stft, energy_matrix, energy_mean, start=True):
    ...
    if voice:
        #print("Habla detectada en el índice: {}".format(voice_start))
        stft = stft[voice_start:]
        
    else:
        print("No se detecta el habla.")
    return stft

def rednoise(samples_recording,samples_noise, sampling_rate):
    '''
    calcula la potencia de la señal de ruido y resta que
    de la grabación
    
    devuelve las muestras de grabación con ruido reducido
    '''
    
    #1) del dominio del tiempo al dominio de la frecuencia:
    #Obtener la transformación de Fourier de corto tiempo (STFT) del ruido y la grabación
    ...
    #3) calcular la media de la potencia, y la variación de la potencia del ruido
    power_mean_n = get_mean_bandwidths(power_n)
    power_var_n = get_var_bandwidths(power_n)
    
    #4) restar el ruido de la grabación:
    #utilizando la comprensión de la lista para trabajar a través de todas las muestras de grabación
    stft_r_rednoise = np.array([subtract_noise(power_mean_n,power_var_n,power_r[i],stft_r[i]) for i in range(stft_r.shape[0])])

    #5) detectar el habla y donde cuando comienza:
    ...
    #Guardar esto para ver si funcionó:
